DECLARE
  ...
  ln_beh_oper_stage_id3   NUMBER;
  ln_beh_oper_stage_id4   NUMBER;
  ln_beh_oper_stage_id5   NUMBER;
  ...
  ln_operation_type_id1   VARCHAR2(60) := 'activeChangeAccount';
  ln_project_owner        VARCHAR(200) := 'TIC Wilson Pozo <wpozo@claro.com.ec>';
  pr_suffix               VARCHAR2(20) := 'CPAYMET_QW';
  lv_desc_camp            VARCHAR2(60) := '[22921] ';
  lv_description          VARCHAR2(100) := 'Change Payment Method Quickwin';
  lv_stage                VARCHAR2(60) := 'OPERATION_INVOKE';
  lv_validation           VARCHAR2(60) := 'DYNAMIC_VALIDATION';
--SUBSCRIBER PROPERTIES NAMES
  SP_DATE_LAST_CHANGE           VARCHAR2(60) := 'DATE_LAST_CHANGE';
  SP_DAYS_LAST_CHANGE           VARCHAR2(60) := 'DAYS_LAST_CHANGE';
  SP_ACCOUNT_ID                 VARCHAR2(60) := 'ACCOUNT_ID';
  SP_ACCOUNT_CODE               VARCHAR2(60) := 'ACCOUNT_CODE';
  SP_ACCINF_SMSNO               VARCHAR2(60) := 'ACCINF_SMSNO';
  SP_ACCINF_EMAIL               VARCHAR2(60) := 'ACCINF_EMAIL';
  SP_ACCINF_LASTNAME            VARCHAR2(60) := 'ACCINF_LASTNAME';
  SP_ACCINF_SECONDLASTNAME      VARCHAR2(60) := 'ACCINF_SECONDLASTNAME';
  SP_ACCINF_FIRSTNAME           VARCHAR2(60) := 'ACCINF_FIRSTNAME';
  SP_ACCINF_FULLNAME            VARCHAR2(60) := 'ACCINF_FULLNAME';
  SP_ACCINF_PAYTYPE             VARCHAR2(60) := 'ACCINF_PAYTYPE';
  SP_ACCINF_PAYMETHOD           VARCHAR2(60) := 'ACCINF_PAYMETHOD';
  SP_ACCINF_STATUS              VARCHAR2(60) := 'ACCINF_STATUS';
  SP_ACCINF_STATUSTIME          VARCHAR2(60) := 'ACCINF_STATUSTIME';
  SP_ACCINF_INITIALBALANCE      VARCHAR2(60) := 'ACCINF_INITIALBALANCE';
  SP_ACCINF_BILLLANGUAGE        VARCHAR2(60) := 'ACCINF_BILLLANGUAGE';
  SP_ACCINF_BILLCYCLETYPE       VARCHAR2(60) := 'ACCINF_BILLCYCLETYPE';
  SP_ACCINF_CREDITLIMITTYPE     VARCHAR2(60) := 'ACCINF_CREDITLIMITTYPE';
  SP_ACCINF_CREDITLIMITVALUE    VARCHAR2(60) := 'ACCINF_CREDITLIMITVALUE';
  SP_ACCINF_BILL_ID             VARCHAR2(60) := 'ACCINF_BILL_ID';
  SP_ACCINF_BILL_TYPE           VARCHAR2(60) := 'ACCINF_BILL_TYPE';
  SP_ACCINF_BILL_CONTENTMODE    VARCHAR2(60) := 'ACCINF_BILL_CONTENTMODE';
  SP_ACCINF_PROPERTY_ID         VARCHAR2(60) := 'ACCINF_PROPERTY_ID';
  SP_ACCINF_PROPERTY_VALUE      VARCHAR2(60) := 'ACCINF_PROPERTY_VALUE';
  SP_ACCINF_PAYCHANNEL_ID       VARCHAR2(60) := 'ACCINF_PAYCHANNEL_ID';
  SP_ACCINF_PAYMODE             VARCHAR2(60) := 'ACCINF_PAYMODE';
  SP_ACCINF_BANKACCNAME         VARCHAR2(60) := 'ACCINF_BANKACCNAME';
  SP_ACCINF_EFFECTIVETIME       VARCHAR2(60) := 'ACCINF_EFFECTIVETIME';
  SP_ACCINF_EXPIRYTIME          VARCHAR2(60) := 'ACCINF_EXPIRYTIME';
  SP_ACCINF_PAYPLAN_ID          VARCHAR2(60) := 'ACCINF_PAYPLAN_ID';
  SP_ACCINF_PAYPLAN_TYPE        VARCHAR2(60) := 'ACCINF_PAYPLAN_TYPE';
  SP_ACCINF_PAYPLAN_EFFTIME     VARCHAR2(60) := 'ACCINF_PAYPLAN_EFFTIME';
  SP_ACCINF_PAYPLAN_EXPTIME     VARCHAR2(60) := 'ACCINF_PAYPLAN_EXPTIME';
  SP_ACC_ADDRESS_ID             VARCHAR2(60) := 'ACC_ADDRESS_ID';
  SP_ACC_ADDRESS_FORMATEDTYPE   VARCHAR2(60) := 'ACC_ADDRESS_FORMATEDTYPE';
  SP_ACC_ADDRESS_1              VARCHAR2(60) := 'ACC_ADDRESS_1';
  SP_ACC_ADDRESS_2              VARCHAR2(60) := 'ACC_ADDRESS_2';
  SP_ACC_ADDRESS_3              VARCHAR2(60) := 'ACC_ADDRESS_3';
  SP_ACC_ADDRESS_4              VARCHAR2(60) := 'ACC_ADDRESS_4';
  SP_ACC_ADDRESS_6              VARCHAR2(60) := 'ACC_ADDRESS_6';
  SP_ACC_ADDRESS_9              VARCHAR2(60) := 'ACC_ADDRESS_9';
  SP_ACC_ADDRESS_10             VARCHAR2(60) := 'ACC_ADDRESS_10';
  SP_ACC_ADDRESS_12             VARCHAR2(60) := 'ACC_ADDRESS_12';
  SP_CUSTOMER_ID_CPM            VARCHAR2(60) := 'CUSTOMER_ID_CPM';
  SP_BANK_CODE_CPM              VARCHAR2(60) := 'BANK_CODE_CPM';
  SP_PAYMENT_METHOD_TYPE_CPM    VARCHAR2(60) := 'PAYMENT_METHOD_TYPE_CPM';
  SP_ACCOUNT_TYPE_CPM           VARCHAR2(60) := 'ACCOUNT_TYPE_CPM';
  SP_BANK_ACCOUNT_NUMBER_CPM    VARCHAR2(60) := 'BANK_ACCOUNT_NUMBER_CPM';
  SP_SERVICE_NUMBER_CPM         VARCHAR2(60) := 'SERVICE_NUMBER_CPM';
  SP_IDENTIFICATION_NUMBER_CPM  VARCHAR2(60) := 'IDENTIFICATION_NUMBER_CPM';
  SP_IDENTIFICATION_TYPE_CPM    VARCHAR2(60) := 'IDENTIFICATION_TYPE_CPM';
  SP_BUSINESS_SERIAL_NUMBER     VARCHAR2(60) := 'BUSINESS_SERIAL_NUMBER';
  SP_CODE_CREDIT_ENGINE_CPM     VARCHAR2(60) := 'CODE_CREDIT_ENGINE_CPM';
  SP_RESP_MESSAGE               VARCHAR2(60) := 'RESP_MESSAGE';
  SP_CHANNELID                  VARCHAR2(60) := '$CHANNELID$';
...
VALUES(ln_campaign_id, lv_description, lv_desc_camp || lv_description, ln_behaviour_id, ln_project_owner);
...
--SUBCRIBER_PROPERTIES
  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_DATE_LAST_CHANGE;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_DATE_LAST_CHANGE, lv_desc_camp || 'Fecha ultimo cambio forma de pago', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_DAYS_LAST_CHANGE;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_DAYS_LAST_CHANGE, lv_desc_camp || 'Dias ultimo cambio forma de pago', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCOUNT_ID;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCOUNT_ID, lv_desc_camp || 'ID de cuenta', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCOUNT_CODE;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCOUNT_CODE, lv_desc_camp || 'Codigo de cuenta', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_SMSNO;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_SMSNO, lv_desc_camp || 'Numero SMS de la cuenta de usuario', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_EMAIL;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_EMAIL, lv_desc_camp || 'Email de la cuenta de usuario', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_LASTNAME;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_LASTNAME, lv_desc_camp || 'Apellido de la cuenta de usuario', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_SECONDLASTNAME;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_SECONDLASTNAME, lv_desc_camp || 'Segundo apellido de la cuenta de usuario', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_FIRSTNAME;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_FIRSTNAME, lv_desc_camp || 'Primer nombre de la cuenta de usuario', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_FULLNAME;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_FULLNAME, lv_desc_camp || 'Nombre completo de la cuenta de usuario', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_PAYTYPE;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_PAYTYPE, lv_desc_camp || 'Tipo de pago', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_PAYMETHOD;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_PAYMETHOD, lv_desc_camp || 'Metodo de pago', 'A');
  END IF;
  
  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_STATUS;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_STATUS, lv_desc_camp || 'Estado de la cuenta de usuario', 'A');
  END IF;
  
  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_STATUSTIME;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_STATUSTIME, lv_desc_camp || 'Fecha de la modificacion del estado', 'A');
  END IF;
  
  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_INITIALBALANCE;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_INITIALBALANCE, lv_desc_camp || 'Saldo inicial de la cuenta de usuario', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_BILLLANGUAGE;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_BILLLANGUAGE, lv_desc_camp || 'Idioma de la factura de la cuenta de usuario', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_BILLCYCLETYPE;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_BILLCYCLETYPE, lv_desc_camp || 'Tipo de ciclo de la cuenta de usuario', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_CREDITLIMITTYPE;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_CREDITLIMITTYPE, lv_desc_camp || 'Codigo de limite de credito de la cuenta de usuario', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_CREDITLIMITVALUE;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_CREDITLIMITVALUE, lv_desc_camp || 'Valor limite de credito', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_BILL_ID;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_BILL_ID, lv_desc_camp || 'ID del medio de factura', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_BILL_TYPE;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_BILL_TYPE, lv_desc_camp || 'Tipo de medio de factura', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_BILL_CONTENTMODE;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_BILL_CONTENTMODE, lv_desc_camp || 'Tipo de contenido', 'A');
  END IF;
  
  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_PROPERTY_ID;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_PROPERTY_ID, lv_desc_camp || 'Identificador de propiedad', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_PROPERTY_VALUE;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_PROPERTY_VALUE, lv_desc_camp || 'Valor de la propiedad', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_PAYCHANNEL_ID;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_PAYCHANNEL_ID, lv_desc_camp || 'ID del canal de pago', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_PAYMODE;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_PAYMODE, lv_desc_camp || 'Codigo de modo de tipo de pago', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_BANKACCNAME;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_BANKACCNAME, lv_desc_camp || 'Nombre de la cuenta de banco', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_EFFECTIVETIME;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_EFFECTIVETIME, lv_desc_camp || 'Tiempo efectivo', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_EXPIRYTIME;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_EXPIRYTIME, lv_desc_camp || 'Tiempo de expiracion', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_PAYPLAN_ID;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_PAYPLAN_ID, lv_desc_camp || 'ID plan de pago', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_PAYPLAN_TYPE;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_PAYPLAN_TYPE, lv_desc_camp || 'Tipo de plan de pago', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_PAYPLAN_EFFTIME;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_PAYPLAN_EFFTIME, lv_desc_camp || 'Tiempo efectivo plan de pago', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCINF_PAYPLAN_EXPTIME;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCINF_PAYPLAN_EXPTIME, lv_desc_camp || 'Tiempo expiracion plan de pago', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACC_ADDRESS_ID;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACC_ADDRESS_ID, lv_desc_camp || 'ID direccion de usuario', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACC_ADDRESS_FORMATEDTYPE;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACC_ADDRESS_FORMATEDTYPE, lv_desc_camp || 'Direccion tipo de formato', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACC_ADDRESS_1;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACC_ADDRESS_1, lv_desc_camp || 'Direccion 1 del usuario', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACC_ADDRESS_2;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACC_ADDRESS_2, lv_desc_camp || 'Direccion 2 del usuario', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACC_ADDRESS_3;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACC_ADDRESS_3, lv_desc_camp || 'Direccion 3 del usuario', 'A');
  END IF;
  
  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACC_ADDRESS_4;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACC_ADDRESS_4, lv_desc_camp || 'Direccion 4 del usuario', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACC_ADDRESS_6;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACC_ADDRESS_6, lv_desc_camp || 'Direccion 6 del usuario', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACC_ADDRESS_9;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACC_ADDRESS_9, lv_desc_camp || 'Direccion 9 del usuario', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACC_ADDRESS_10;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACC_ADDRESS_10, lv_desc_camp || 'Direccion 10 del usuario', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACC_ADDRESS_12;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACC_ADDRESS_12, lv_desc_camp || 'Direccion 12 del usuario', 'A');
  END IF;
  
  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_CUSTOMER_ID_CPM;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_CUSTOMER_ID_CPM, lv_desc_camp || 'ID de cliente Change Payment Account', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_BANK_CODE_CPM;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_BANK_CODE_CPM, lv_desc_camp || 'Codigo Banco Change Payment Account', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_PAYMENT_METHOD_TYPE_CPM;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_PAYMENT_METHOD_TYPE_CPM, lv_desc_camp || 'Metodo Pago Change Payment Account', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_ACCOUNT_TYPE_CPM;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_ACCOUNT_TYPE_CPM, lv_desc_camp || 'Tipo Cuenta Change Payment Account', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_BANK_ACCOUNT_NUMBER_CPM;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_BANK_ACCOUNT_NUMBER_CPM, lv_desc_camp || 'Numero Cuenta Change Payment Account', 'A');
  END IF;
  
  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_SERVICE_NUMBER_CPM;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_SERVICE_NUMBER_CPM, lv_desc_camp || 'Numero Servicio Change Payment Account', 'A');
  END IF;
  
  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_IDENTIFICATION_NUMBER_CPM;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_IDENTIFICATION_NUMBER_CPM, lv_desc_camp || 'Numero de identificacion del cliente', 'A');
  END IF;
  
  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_IDENTIFICATION_TYPE_CPM;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_IDENTIFICATION_TYPE_CPM, lv_desc_camp || 'Tipo de identificacion del cliente', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_BUSINESS_SERIAL_NUMBER;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_BUSINESS_SERIAL_NUMBER, lv_desc_camp || 'Codigo unico de orden de transaccion', 'A');
  END IF;
  
  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_CODE_CREDIT_ENGINE_CPM;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_CODE_CREDIT_ENGINE_CPM, lv_desc_camp || 'Codigo de respuesta del motor de credito', 'A');
  END IF;

  select count(SUBSCRIBER_PROPERTY_ID) into SP_COUNTER from SUBSCRIBER_PROPERTIES where SUBSCRIBER_PROPERTY_ID = SP_RESP_MESSAGE;
  IF SP_COUNTER = 0 THEN
    insert into subscriber_properties(subscriber_property_id, description, status) values(SP_RESP_MESSAGE, lv_desc_camp || 'Mensaje de respuesta de Change Account', 'A');
  END IF;
...
"businessCode": "ModifyAccountInfo",
  "bussinessSerialNo": "<xsl:value-of select="//_businessSerialNo"/>",
...
VALUES((SELECT Nvl(Max(IR.invoke_response_evaluation_id), 0) + 1 FROM invoke_response_evaluation IR), ln_invoke_id1, 10, '//*[local-name()="Account"]/*[local-name()="account"]/*[local-name()="accountInformation"]/*[local-name()="id"]/text()', 'ND', null, null, '!=', 'YES', 'No existe ningun registro de cuenta', 'A');
  ...
  INSERT INTO invoke_response_evaluation(invoke_response_evaluation_id, invoke_id, order_by, xpath, evaluation_type, expected_value, value_type, evaluation_operator, success, failure_message, status)
VALUES((SELECT Nvl(Max(IR.invoke_response_evaluation_id), 0) + 1 FROM invoke_response_evaluation IR), ln_invoke_id3, 10, '//*[local-name()="response"]/*[local-name()="idResult"]/text()', 'FV', '0', 'NUMERIC', '==', 'YES', '[' || SP_RESP_MESSAGE || ']', 'A');
